...
void qorus_QjobServerSession(bool qorus_dbg_internals, QoreProgram* qpgm, ExceptionSink* xsink);
...
void qorus_ClusterQorusJob(bool qorus_dbg_internals, QoreProgram* qpgm, ExceptionSink* xsink);
...
void qorus_qorus_job_system(bool qorus_dbg_internals, QoreProgram* qpgm, ExceptionSink* xsink);

void qorus_qjob(bool qorus_dbg_internals, QoreProgram* qpgm, ExceptionSink* xsink);
...
int main(int argc, char* argv[]) {
   ...
   for (unsigned i = 0; i < NUM_INT_MODULES; ++i) {
    ...
    qorus_qjob(qorus_dbg.internals, qpgm, &xsink);
    ...
    qorus_QjobServerSession(qorus_dbg.internals, qpgm, &xsink);
    ...
    qorus_ClusterQorusJob(qorus_dbg.internals, qpgm, &xsink);
    ...
    qorus_qorus_job_system(qorus_dbg.internals, qpgm, &xsink);

    qpgm->parseCommit(&xsink, &xsink, QORUS_WARN_MASK);

    int rc = 0;
    if (!xsink.isEvent()) {
        qpgm->runClass("QJob", &xsink);
    }
