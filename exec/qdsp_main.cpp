...
void qorus_qdsp(bool qorus_dbg_internals, QoreProgram* qpgm, ExceptionSink* xsink);
...
int main(int argc, char* argv[]) {
    ...
    qorus_dbg_t qorus_dbg = qorus_parse_options(argc, argv, po, {
        {"qorus.oracle-datasource-pool",
            [oracle_dsp] (const std::string& val, int64& po) mutable -> void {
                oracle_dsp = q_parse_bool(val.c_str());
            }
        }
    });
    ...
    // set define for the QDSP server
    qpgm->parseDefine("QdspServer", true);
    ...
    if (oracle_dsp) {
        // see if this is an oracle datasource
        QoreString connstr;
        if (!SystemEnvironment::get("CONNSTR", connstr) && !strncmp("oracle:", connstr.c_str(), 7)) {
            if (my_load_mod("oracle", qpgm) || my_load_mod("OracleExtensions", qpgm))
                return 1;
        } else
            oracle_dsp = false;
    }
   ...
   for (unsigned i = 0; i < NUM_INT_MODULES; ++i) {
    ...
    qorus_qdsp(qorus_dbg.internals, qpgm, &xsink);
