...
qorus_uid_gid_files_exist() {
    [ -e "${QORUS_INIT_STATE_DIR}/QORUS_UID" ] && [ -e "${QORUS_INIT_STATE_DIR}/QORUS_GID" ] && return 0
    return 1
}

qorus_systemdb_is_set() {
    [ -e "${OMQ_DIR}/etc/options" ] && grep -E "^qorus\.systemdb" ${OMQ_DIR}/etc/options > /dev/null 2>&1 && return 0
    return 1
}

qorus_schema_prepared() {
    [ -e "${QORUS_INIT_STATE_DIR}/schema_prepared" ] && return 0
    return 1
}

qorus_network_key_readable() {
    [ -e "${OMQ_DIR}/etc/network.key" ] && [ -r "${OMQ_DIR}/etc/network.key" ] && return 0
    return 1
}

qorus_init_finished() {
    [ -e "${QORUS_INIT_STATE_DIR}/init_finished" ] && return 0
    return 1
}
