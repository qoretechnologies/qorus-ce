...
qorus_user_exists() {
    cat /etc/passwd | grep qorus > /dev/null 2>&1
}

# return if qorus group already exists
qorus_group_exists() {
    cat /etc/group | grep qorus > /dev/null 2>&1
}
...
add_qorus_user_group() {
    ...
    if [ "${QORUS_UID}" != "0" ] && ! qorus_user_exists; then
        ...
        useradd -r -o -u ${QORUS_UID} -g ${QORUS_GID} -M -d ${OMQ_DIR} -s /bin/bash qorus
        echo "if [ -f ${OMQ_DIR}/env.sh ]; then . ${OMQ_DIR}/env.sh; fi" >> ${OMQ_DIR}/.bashrc
    fi
}
...
https_cert_option_is_set() {
    grep -E -q -e "^qorus.(http-secure-certificate|http-secure-server.*cert=)" ${OMQ_DIR}/etc/options > /dev/null 2>&1
}

# return if Qorus HTTPS cert private key option is set
https_privkey_option_is_set() {
    grep -E -q -e "^qorus.(http-secure-private-key|http-secure-server.*key=)" ${OMQ_DIR}/etc/options > /dev/null 2>&1
}

# return if Qorus HTTPS server option is set
https_server_option_is_set() {
    grep -q -e "^qorus.http-secure-server" ${OMQ_DIR}/etc/options > /dev/null 2>&1
}
...
db_tablespace_set() {
    grep -e "^qorus-client.omq-data-tablespace" ${OMQ_DIR}/etc/options > /dev/null 2>&1
}

# return if Qorus DB tablespace option is set and matches $OMQ_DB_TABLESPACE
db_tablespace_matches() {
    grep -e "^qorus-client.omq-data-tablespace" ${OMQ_DIR}/etc/options | grep "${OMQ_DB_TABLESPACE}" > /dev/null 2>&1
}
...
systemdb_option_set() {
    grep -e "^qorus.systemdb" ${OMQ_DIR}/etc/options > /dev/null 2>&1
}
...
dbparams_contains_omq_ds() {
    grep -e "^omq=" ${OMQ_DIR}/etc/dbparams > /dev/null 2>&1
}

# return if DB type set in OMQ_DB_TYPE env var is valid
db_type_is_valid() {
    echo "${OMQ_DB_TYPE}" | grep -e "\(\(my\|pg\)sql\|oracle\)" > /dev/null 2>&1
}
