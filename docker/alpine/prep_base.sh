...
export LD_LIBRARY_PATH=${ORACLE_INSTANT_CLIENT}:${INSTALL_PREFIX}/lib:${INSTALL_PREFIX}/lib:${LD_LIBRARY_PATH}
...
export CMAKE_MODULES_BUILDABLE="jni json linenoise magic mysql odbc openldap oracle pgsql process python ssh2 sysconf uuid xml xmlsec yaml zmq"
...
# create symlinks required for Oracle
ln -s /lib/libc.musl-x86_64.so.1 /buildroot/usr/lib/libresolv.so.2
ln -s /lib/ld-musl-x86_64.so.1 /buildroot/usr/lib/ld-linux-x86-64.so.2
